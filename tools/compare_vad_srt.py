...
A dev tool, not a test helper: it answers "how far apart are these two
subtitle timelines" when comparing a VAD run against a reference.

This module is usable in two ways:
1) Callable API:
   from tools.compare_vad_srt import compare_srt_files
2) CLI client:
   python -m tools.compare_vad_srt a.srt b.srt

Its own tests live next to it (`tools/test_compare_vad_srt.py`) and are not
collected by the default suite -- see `tools/`'s on-demand policy.
"""

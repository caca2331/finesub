...
> **2026-09-03：整节搬到 [`batch-scheduler.md`](batch-scheduler.md)。** 那一节讲的是
> `scheduler.py` / `batch_state.py` 的**现行**契约（三 bin 的分工、llm bin 的并发语义、
> 运行期队列面与控制面、`--resume-batch`、产物唯一性、失败与中断语义），与本文其余部分
> 「已移除的单文件分片」不是一个体裁，留在这里只会让人以为它也已作废。
>
> 本文仍然承载的、且**仍然成立**的是：asr 恒为 1 的并发实测依据（下面「Worker 数量」与
> 「实测标定」两节）、stdio 背压根因、intra-op 线程预算、语义分组边界。
...
`--gpu-tier` 或开发参数 `--wt-workers` 通过 `task_workers` 改变 shard 划分，进而可能改变
...
能消化「batch 队列尾部只剩一个长文件」的空闲，但要求改写 `scheduler.py` 的 asr bin 与

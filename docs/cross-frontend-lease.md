...
> **2026-09-03，桌面端已从本仓剥离（0.5.0）。** 机制原样保留：协议对任何第二个进程都成立——
> 两个 `finesub` 运行、一次 `relocate` 与一个在跑的任务、以及仍装着 0.4.x 桌面端的机器。
> 下文里的「桌面 worker / JobManager」是设计时的第二前端，读作「任何第二个持有者」即可；
> `frontend` 字段的取值表（`desktop|cli`）也因此不动。

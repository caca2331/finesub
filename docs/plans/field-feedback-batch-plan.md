...
5. **隐私——这是本节最贵的一段**：
   - 硬排除 `.env*` 与任何 key 形态的文件；`config.toml` 只收**白名单的节**，节里再剔掉名字像
     凭据的键并抹掉 URL 里的 `user:pass@`。⚠ **不做逐键白名单**（owner 2026-09-04，复审三次提、
     三次都不做）：这个包是开发阶段用户**看过 MANIFEST 之后手动发给开发者**的东西，不是自动上传；
     节里会进摘录的标量键本来就只有十来个，而 reviewer 举的「值里藏凭据」（`note = "token …"`）
     两种方案都拦不住。差不多就行，别再往这里加机制。
...
- **§1 不为「镜像未来可能支持 Xet」留探测逻辑。** 一次探测换不到任何东西：关掉 Xet 的代价是零
  （见 §1.3），而探测本身是新的失败面。
- **§4 的收集策略保持「目录扫描 + 前缀归属 + 后缀黑名单」，不改成从管线路径解析器派生确定集合**
  （owner 2026-09-04）。复审第三轮把三件事归为同一根因（`a` 与 `a-b` 共用 `-o` 目录时前缀互相
  收进；日志 stem 归一化后 `a b` 与 `a-b` 撞名；`--with-source` 对默认布局找不到产物目录之外的
  源文件）——都成立，但都是显式 `-o` 进共享目录、或六十字以上同前缀输入名这类边角，而包在发出前
  由用户按 MANIFEST 逐文件看过。要收紧的话正确形态是复用 `finesub_bootstrap.artifacts` 的后缀清单
  按名字精确匹配、源文件从 `tasks.json` 的 `input` 取——记在这里，等它真咬到人再做。后缀黑名单
  这边照管线的 `_VIDEO_EXTENSIONS` 补齐了 `.wmv/.mpg/.mpeg`（2026-09-04）。
...
| 7 | （2026-09-04）§4 的配置摘录**不做逐键白名单**，收集策略**不改成从解析器派生**——「开发阶段传给我的而已，差不多就行」。两条写进 §4.3 与 §6，reviewer 再提按此回复 |

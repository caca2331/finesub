...
逐批实施记录与偏离表迁到了 `docs/archive/refactor-plan-2026-08.md`（在 `dev` 上被跟踪，
但不随仓库发布）——那份是过程台账，读它只为考古。本文只留**仍然要做的事**、**判定不做的事**，
以及**做同类改动前值得先读的几条**。
...
那一轮里同一类失败反复出现——**改动全绿、渲染正常、引用指向空气**。前四条来自那一轮，第五条是 2026-09-01 又撞了三次才补上的。都是实证：
...
5. **守卫的扫描面本身就是守卫的一部分；它没看的地方，永远不会红。** 一条守卫的说服力上限
   是「它扫了什么」，而不是「它断言了什么」——而扫描面是最容易被默认成理所当然的部分。
   2026-09-01 一天之内同一形状撞了三次：
   - 选项默认值棘轮按**同名相交**比较 argparse 与后端签名，而 `--model`/`--gap`/
     `--separator-rate` 两侧拼写不同（`_ROW_ALIASES`），于是**从未被比较过**，其中两个一直
     带着重复默认值；
   - 同一条棘轮扫的是「argparse ↔ 签名」，**从不看行**，于是 URL 分支里 12 处
     `opts.get(k) or <字面量>` 这类第三份副本天生在视野外（评审发现，其中一处 `or` 换行、
     连 grep 都看不见——**AST 比正则更该是守卫的默认形态**）；
   - `test_doc_links` 验「链接指向的文件存在」，验不了「它有没有讲那件事」，于是两处
     「细节见 X.md」的空头承诺一直绿着。
   改一条守卫时先问：**它现在没扫到什么？** 扩大扫描面通常比加断言更值钱。⚠ 也有反面：
   扩面后第一版把 `opts.get("task_artifact_dir") or paths.task_artifact_dir` 误报了——那是
   派生值不是默认值副本。**扩面要连同「什么不算违规」一起写死**，否则新守卫会被调低到没人信。

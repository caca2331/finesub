...
### referee 设备（五个问题，不是一个）

`referee_device` 依次问五件事。它们曾经是同一个问题——「ASR 在不在 CUDA 上」——而 2026-09-02
...
| 4 | **纸面余量**：pool 真的在卡上吗？占多少 | 下面这张表 | cpu |
| 5 | **卡上余量**：此刻真的还剩这么多吗 | `device.free_vram_gib()`（驱动） | cpu，并回调 `live_vram_veto` 让调用方说后果 |
...
第 5 问是 2026-09-04 加的，补的是第 4 问的一个盲点：**档位是预算，不是测量**。它说的是「够格
用这个档的机器应该有多少空闲」，`auto` 从卡的容量推出它，正是为了让它跨运行稳定——但没有任何
一环去看**此刻**真的剩多少。于是一张开着游戏的 `standard` 卡上，第 4 问照样答「还余 4.43 GiB」，
而驱动手里只有 2.4 GiB，两个模型仍然一起上卡。接下来发生的不是能被接住的 OOM：裁判的
`Module.to(cuda)` 与 CTranslate2 的 encoder 相撞，解码要么慢到分钟级且没有任何事件可看，要么
进程在 group 中途吃访问违例——**CT2 两种都报不出来，因为它是 abort**。

⚠ **第 5 问只否决放置，永远不参与选档。** 实测值随机器上开着什么而摆动，而档位决定分离器的
worker 数、进而决定分块边界——拿它选档会让同一份素材在不同日子产出不同的 artifact 边界
（`free_vram_gib` 的 docstring、`resources.warn_if_vram_is_short`）。**拿不到 CUDA 答案时保持
档位的结论**，与本模块其余「未知」同向。

⚠⚠ **而且它只问一个调用点**，因为「放置只改速度不改输出」这句话**当初写错了**（2026-09-04
复审指出并更正）。理由如下，动这一族之前必须先读懂：

生产的 dtype 是 **CUDA 上 `bfloat16`、CPU 上 `float32`**（`qwen_referee._load_model`）。本文
下面那句「CPU 上用 float32……输出逐字相同」比的是 **CPU-float32 对 CPU-bf16/fp16**，
**不是 CPU 对 GPU**——仓库里没有任何一处证明后一对相等。所以**换设备就是换数值路径**。

| 调用点 | 它的答案影响什么 | 问不问第 5 问 |
| --- | --- | --- |
| `redecode_referee_device`（语言票重解） | 喂 `maybe_redecode` 的采纳判据 → **替换字幕**，且默认开 | **不问**。让实测显存在这里选设备，等于让「机器上此刻还开着什么」改变最终字幕 |
| `referee_warm_device`（解码期间预热） | 只决定要不要提前加载；尾部裁判随后由 `tail_verify_device` 另行放置 | **问**。否决只丢掉约 3 s 加载，**改不了任何产出输出的设备**，上报因此是 debug |
| 尾部裁判（`tail_verify_device`） | 喂 `--qwen-verify` 的证据 | **不问**：池已关闭无可撞之物，而那个预算决定走不走 `FixedShapeDecoder`，两条解码路不 bit-exact |

**owner 决定（2026-09-04）：inline 裁判在 lease 内加载可能与 CT2 撞卡，接受这个风险。**
理由是档位系统本来就在**告诉用户要预留多少空闲显存**，不够时 `gpu-vram-short` 已经在提醒；
用户不腾出来是他那侧的取舍。为它买单的两条路都更贵：把 inline 固定放 CPU 是在**所有**这类机器上
换数值路径（代价与上面同类，且是必然而非偶发），触发时 swap Whisper 已被本文「不做 swap」否过。
⚠ 而这条风险**是既有的**——报告过的那次崩溃在预热路径，那条由第 5 问守着。

⚠ **档位本身早就在决定 CPU/GPU**（`entry` 落 CPU、`standard` 起共驻），所以 CPU/GPU 若真有差异，
**今天已经在机器之间造成差异**了——那是一个既有的未决问题（见下「待标定」）。第 5 问不能做的是
在此之上再叠一层**同一台机器、两次运行之间**的差异：前者稳定可复现，后者不是。

**尾部裁判仍按解析后的 ASR 设备派生**——⚠ 2026-09-04 曾把它改成走这个 oracle，**当天又撤回**。

改的理由是：`device` 读到 "cpu" 有两种含义（用户要求 / CT2 用不了这张卡），后者对 torch 没有
发言权，于是一次 CT2 侧的回退会把裁判一起拖下卡。那个理由**完全是速度**，前提是「放哪只改
它花多久」。前提不成立（见上），而尾部证据也**不是惰性的**：`stabilization` 读它来决定噪声腿
的删除要不要撤销，也就是一行字幕留不留。唯一的动机没了，改动跟着撤。

⚠ **要重新提这件事，先做「待标定」里那份 CPU-float32 对 GPU-bf16 的对照**，不要再引用那句
CPU 内部的比较。

⚠ 曾经这条 warning 由 oracle 自己发、三个调用点共用一套说辞，于是默认 `auto` 下一张占满的卡会
**连发两条**，其中预热点那条的三句话（「改在 CPU 上跑」「校验会明显变慢」「重跑会自动回到显卡」）
**全部不成立**。

⚠ **`pool_resident` 说的是「实测的那个自由值里，池是不是已经被买过了」**：`referee_warm_device`
在 `FwRefineModelPool.warm` **之后**问，所以传 `True`；建池之前问的话必须把 Whisper 也从实测值
里买下来（默认 `False`）。搞反了不是舍入误差——池是两者中更大的那个模型，假 `True` 会超订，
假 `False` 会把裁判从它本该共驻的档位上赶下卡。**⚠ 今天只有预热点问第 5 问，所以 `False` 这一
支在生产里没有调用者**；它留着是因为顺序即前提——哪天又有一个建池之前的调用点要问，忘了传就错。
...
**裁判加载失败按档位分流（2026-09-04）**：这条路径上「证据没拿到」原本有两种出口，都保留原
解码——`no-evidence`（裁判什么都没听出来）与 `redecode-stalled`（强制重解没有推进，代码注释原话
是「the safe exit is keeping the original decode, not killing the run」）。**裁判加载失败是第三种**，
而它此前是唯一一个会打死整趟运行的，连默认档也一样。

现在 `auto` 与 `on` 分流，判据就是两档的定位：

| 档位 | 定位 | 裁判加载失败时 |
| --- | --- | --- |
| `auto` | 顺手的默认改善，用户没点名要 | 发一条 `lang-redecode-failed` warning（impact 与「transformers 缺失」那支同为「语言票翻转窗口不会被重解」），**本轮后续不再尝试**，事件记 `rejected: referee-unavailable` |
| `on` | 用户明确要求 | 原样抛出，运行结束——与 `--qwen-verify on` 同口径 |

⚠ **降级不是「不管证据直接按众数重解」**。触发条件只是「这一组的票与滚动众数不符」，那是**怀疑**；
裁判才是把怀疑变成决定的东西。真正双语的素材里这一组的票是对的、众数在这里不适用，强行按众数
重解会**改坏一段正确的解码**——这正是上面那道硬门存在的理由。

⚠ 影响面只有**四格中的一格**：`audit` 只在两个 `on` 格里跑（`resolve_mode`），而
`auto + --language` 根本不建裁判。所以能走到这条降级的只有 `auto + 自动语言`，也就是默认那格。
...
⚠ **标定之前先看分母**（2026-09-04）：`align_meta.lang_redecode` 现在分三层——`triggers`
（触发了几次，含一切）、**`adjudicated`**（其中规则真的判了几次）、`adopted`（其中采纳了几次），
外加 `rejected` 的按原因直方图。**任何采纳率都要除以 `adjudicated`**，不能除以 `triggers`：
后者含着两类「规则根本没跑成」的事件——`referee-unavailable`（我们的故障，权重没加载起来）与
`no-evidence`（素材的性质，裁判没听到可判的东西）。拿 `triggers` 当分母，会让一次加载失败看起来
像规则自己拒绝了。原因常量在 `lang_redecode.UNADJUDICATED_REASONS`。

而剩下的三种拒绝原因分属两层，标定时也不能混：`evidence-disagrees` 是**规则判了、说不该救**，
`redecode-stalled` / `redecode-empty` 是**规则说该救、执行没做成**——前者是判据的信号，后者不是。

**owner 决定（2026-09-04）：证据不算充足，但足以让它在生产上开着。** 记在这里是因为这个功能
**真改输出**（重解并替换），而它的两个阈值等的是一批零负例的素材——按「有收益但数据太少就只
打标」的一般口径，它本该降到只标记不动手。owner 权衡后选择保留现状，所以这不是疏漏，是一次
**知情的取舍**；下面的待标定项因此仍然有效，而不是被这个决定作废。⚠ 谁要重新审这一档，先读
本节最后那段方法论教训（405 窗口的 0 FP 是类型盲区给出的假保证），别只看阈值本身。

| 待定 | 现值 | 需要什么 |
| --- | --- | --- |
| **裁判 CPU/GPU 是否等价**（2026-09-04 新增） | 未测 | ⚠ 生产 dtype 是 **CUDA `bfloat16` / CPU `float32`**，而本文上面那句「输出逐字相同」比的是 **CPU-float32 对 CPU-bf16/fp16**，**不是这一对**。档位已经在决定放哪（`entry` 落 CPU、`standard` 起共驻），所以若有差异，**今天就已经在机器之间造成字幕差异**。要量的是同素材两臂：① `adopt/reject` 一致率 ② 强制语言一致率 ③ 最终字幕差异。⚠ 在量清之前，**不得**让任何**实测/环境相关**的量（如实时空闲显存）参与这个放置——那会把机器间差异变成同机两次运行之间的差异 |

...
`usable = 档位的 usable_gib`（即该档要求的空闲显存）。BS 为生产配置值；耗时与显存为本机实测。

> ⚠ **下面两张表的列名是 0.5.0 之前的 `--gpu-budget-gb` 四档（4/8/12/16 GB），不是现行三档。**
> 现行 `entry`/`standard`/`high` 要求的空闲显存是 **3 / 6.5 / 10 GiB**
> （[gpu-profiles.md](gpu-profiles.md)），所以旧的 4 GB 列比 `entry` 宽、8 GB 列比
> `standard` 宽、12 GB 列比 `high` 宽——**逐列都偏松**，不能直接当成新档位的预算读。
> 表本身是实测记录，保留原样；要给现行档位定 B，用下面那条按 item 的显存斜率重算。
> 已经能直接读出的一条：`large-v3` 基线 4.34 GB **超过 `entry` 要求的 3 GiB**，
> 那一档只能用 turbo。
...
- **`combined_group_duration ≤ 30s` 不保证拼接音频 ≤ 30s。** gap padding 会把一部分分组顶过
  编码窗口（实测 29 个分组里 5 个）。driver 自 2026-09-02 起接受超过一窗的音频（只批第一窗，
  余下的窗在回放里顺序补完），**但调用方 `DecodePrefetch` 不把它们送进批**：批出来的第一窗
  少一个 token 就会挪动 seek，整组后面的窗全变成另一段音频（一组 97 s 换了 24 条片段，
  `bench-baselines.md` 22.1）。
...
### 上层组批已接线，默认仍关（2026-09-02）

`transcribe.DecodePrefetch` 实现了下面「可选待办」的第 1、2、4、5、6 条：循环前按顺序取 B 个
group 的第一窗一次批解码、结果按音频字节缓存、循环里命中即用、未命中照旧；长组（第 3 条）
留给顺序路径——批第一窗会让 seek 级联改写整组（`bench-baselines.md` 22.1）。12 份产物扫描：
质量在 B=4/8 过第十节门槛，**端到端只有 1.06×，不到 1.15× 的下限**，所以档位表保持 1，
`--asr-decode-batch` 是显式 opt-in（选 4 即可，8/16 无额外收益、16 质量不过）。driver 自 2026-09-02
起接受 `language=None`（逐窗检测）与超过一窗的音频（只批第一窗）。
...
> ⚠ **上面这段的前提已经失效（2026-08-29）。** 那个 72/28 是旧卡、E14/E15/E16 之前的
> 数字。[`separator-optimization.md`](separator-optimization.md) E16（2026-08-27 新机
> 基线）实测**分离 47% → 39%、VAD+ASR 53% → 61%**，E16 自己写着「这一格是后续优先级
> 的依据」。所以「下一个值得优化的是分离不是 ASR」这句**不再成立**，而分离那一路
> E16 已用三个上限自己封住。
>
> 但**收益仍未知**：同一个 1.73× 的上限约折合 GPU 段 1.35×，实际值取决于那 98 秒里
> 可组批解码占多少——这个数还没量。准确表述是「**ASR 重新成为值得测量的候选**」，
> 而不是「已知能拿多少」。计划与前置见
> [`crispasr-followups.md`](plans/crispasr-followups.md) 批次 C。
...
1. **静态组批 + 波前推进**。一次取 B 个 ≤30s 的 group 组成一批；解码提前结束需要第二次
   ...
   是进程级硬中止，不能试探自适应。
2. **波前组批 + 余量顺序补完**（owner 定，2026-08-31）。CT2 要求整批同一 prompt 形状，
   而依赖 `condition_on_previous_text` 的连续窗（>30s 分组的第 2 窗起）prompt 非空。
   调度上先做**无前缀的那一批**——每个 group 的第一窗，它们的 prompt 都是空的、形状天然
   一致；**带前缀的余量在这一批之后顺序做完**。
   ⚠ 这不只是「省事」，是**依赖顺序决定的**：第 2 窗的 prompt 就是第 1 窗解出来的文本，
   在第一批跑完之前它根本不存在，所以它无论如何都排在后面。
   ⚠ **不要用填充 token 把不同长度的前缀凑齐**：Whisper 的 prompt 文法没有 padding 槽，
   `<|startofprev|>` 之后的任何东西都会被当成「这一窗之前发生过的内容」读进去，
   补进去的就是喂给解码器的虚构。（2026-08-31 一度按「线性补齐 prompt 长度」实现过
   `common_prompt_length`，与 owner 要的调度语义不是一回事，已回退。）
3. **>30s 分组的处理**。`combined_group_audio_seconds ≤ 30s` 已是分组规划的判据，但仍有约
   14% 的组超窗（密集语音找不到切点，最长 73.9s；本次扫描的语料里更多，yui 36 组）。这些组
   目前走顺序路径。要让它们进批，要么为适配编码窗口强制切分 group——独立的质量取舍，见
   [`asr-align.md`](asr-align.md)——要么研究 faster-whisper 自己的 `BatchedInferencePipeline`
   怎么做的：它按 VAD 把音频切成 ≤30 s 的块、每块作为独立窗口解码、并放弃 `condition_on_previous_text`，
   也就是根本不存在跨窗的 seek 级联。**已搁置**（2026-09-02，owner）：组批整体上限约 1.3×，
   不值得为它改分组语义。
...
**人工对听：结案不做**（owner 2026-09-03）。当时留的是相似度最低的 BV1UBjq6fEgb（85.9%，
41 处差异，多为 `かわいい`/`可愛い`、标点这类正字法差异，但有实质分歧，且恰好是 wt 做了
6 次隔离而 fw 一次都没有的素材）。**它已经不是验收项了**：wt 后端早已从代码里移除，听出
哪边更好都回不去，而机械指标该说的都说了（词数与覆盖秒 ±1%、救援活动一致下降无一反例、
生产异常 5 素材共 1 条）。两侧产物仍留在 `out/acceptance/`——将来真要查 fw-refine 的
**系统性**弱点（比如那 6 次隔离对应的段落），素材是现成的，但那是新问题的证据，不是这次
迁移的欠账。

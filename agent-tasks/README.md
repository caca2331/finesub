...
| `release/` ⚠ | 发布新版本的完整流程：版本 bump、orphan `main` 快照与 CI 把关、GitHub Release、PyPI |

⚠ 带 ⚠ 的这份是**维护者专用**，由 `scripts/publish-main.ps1` 从每次公开快照里剥掉，所以
公开仓库里没有它（`main` 的全部历史里也从未有过，2026-09-01 核实）。这里只列名字不给
链接——公开树里点不开的链接比没有更糟。在本仓库的工作副本里它就在同名目录下。

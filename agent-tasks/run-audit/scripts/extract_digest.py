...
#: ⚠ `<mistake_proposals>` was deleted on 2026-09-02 along with the two
#: markdown ledgers: a run from then on never emits it, and the loop below
#: simply finds no block (absence is skipped, never flagged). These rules
#: stay for auditing OLDER artifacts. Style conventions now travel as
#: ordinary entry lines inside `<knowledge_proposals>`.
...
# Mirrors the six fixed sections of node/presets/streamer.toml (strict_sections).
STREAMER_SECTIONS = ("档案", "直播内容", "频道用语", "特点", "人际关系", "待归类")
...
def resolve_layout(run_dir: Path, artifact_dir: Path | None = None) -> dict:
    layout: dict = {"run_dir": run_dir}
    if artifact_dir is not None:
        layout["artifact_dir"] = artifact_dir if artifact_dir.is_dir() else None
        if layout["artifact_dir"] is None:
            flag(f"--artifact-dir 不存在：{artifact_dir}")
        return _resolve_layout_files(run_dir, layout)
    # A run driven with --task-artifact-dir may put artifacts in ANY directory
    # name (the 2026-08-28 agent run used plain `artifacts/`); a "-artifacts"
    # suffix match alone silently empties the whole timeline section. Any
    # subdirectory carrying at least one marker is a candidate.
    dirs = [
        d
        for d in run_dir.iterdir()
        if d.is_dir()
        and (
            d.name.endswith("-artifacts")
            or any((d / name).exists() for name in ARTIFACT_MARKERS)
        )
    ]
    dirs.sort(
        key=lambda d: (
...
def _resolve_layout_files(run_dir: Path, layout: dict) -> dict:
...
def audit_kb(root: Path, out: list[str]) -> None:
    ...
    # Since the 2026-08-22 SQLite migration the readable surface is the derived
    # `rendered/` cache; `<root>/<category>/` is the retired import archive that
    # no run writes to any more. Reading the archive silently audited a frozen
    # copy: entries written since the migration were invisible and its stale
    # files drew FLAGS nobody can act on.
    rendered = root / "rendered"
    migrated = rendered.is_dir()
    base = rendered if migrated else root
    if migrated:
        out.append(f"- 读取面: {rendered}（派生缓存，SQLite 为真相源）")
    for category in ("streamer", "common"):
        ...
        for stem in sorted(files):
            ...
            if category == "streamer":
                # The streamer preset is `strict_sections = true` over these six
                # (node/presets/streamer.toml). `## 经历` was checked here long
                # after the schema dropped it, so every entry drew a FLAG for a
                # section none of them may have.
                for section in STREAMER_SECTIONS:
...
def main() -> int:
    ...
    parser.add_argument(
        "--artifact-dir",
        help="Explicit task artifact dir (for runs driven with --task-artifact-dir)",
    )
    ...
    if args.kb:
        ...
        layout = resolve_layout(
            run_dir, Path(args.artifact_dir) if args.artifact_dir else None
        )
        ...
        if artifact_dir is None or not (Path(artifact_dir) / "task-artifacts.jsonl").exists():
            # Never let this read as a clean run: with no task artifacts the
            # timeline / feedback / attempt sections are all BLANK, and
            # "FLAGS：无" would be a false green (2026-08-28 audit).
            flag(
                "未找到 task-artifacts.jsonl——时间线/feedback/attempt 审计不可用；"
                "若该 run 用了 --task-artifact-dir，请用 --artifact-dir 指认"
            )
        artifact_dir = layout.get("artifact_dir")
        if artifact_dir:

...
2. **跑抽取脚本**（每 run 一次）：
   ...
   ```powershell
   python agent-tasks/run-audit/scripts/extract_digest.py out/reference/<id> --refined <精修.srt>
   python agent-tasks/run-audit/scripts/extract_digest.py --kb knowledge
   ```

   digest = 布局 + 统计 + **correction 时间线** + FLAGS。FLAGS 是嫌疑不是结论；脚本测不了
   语义与「成类」问题。run 用了 `--task-artifact-dir` 时给脚本传 `--artifact-dir` 指认；
   digest 打出「未找到 task-artifacts.jsonl」的 FLAG 时，其余小节全是空转，**不要**把
   「FLAGS 少」当干净。
...
4. **语义抽查**（脚本测不了）：
   ...
   - knowledge proposals：长尾定位、reason 证据。（`mistake proposals` 只在 2026-09-02
     之前的产物里有，那个块与两个台账都已删除。）
   ...
   - **清单外巡检**：忘掉 FLAGS，扫成品 SRT 头/中/尾 + 一份 exchange——字形、覆盖时长、
     语言设置等重大问题常靠这一步发现。
   - **agent 路径的检索状态**：模型在 reasoning/feedback 里自述的「检索不可用/没有来源」
     **不是证据**——assignment 台账才是（`%LOCALAPPDATA%/Temp/finesub-agent-runtime/<hash>/`
     `assignments/<call>/control/state/` 最新 state json 的 per-call 台账；feedback artifact
     自 2026-08-28 起也带 harness 回填的 `retrieval_urls`）。2026-08-28 审计曾据自述误判
     「捏造来源」，台账证伪。

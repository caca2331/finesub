...
来源是历次真实 run 的审计沉淀（首批：2026-07 的一组三连 run）——发现新模式请追加到本文。
...
## B. mistake 台账字段语义类（**仅 2026-09-02 之前的 run**）

> ⚠ 那个块与两个 markdown 台账已删除。审计新 run 时本节整节不适用——风格类的更新
> 现在是 `<knowledge_proposals>` 里写进 `style` 条目的普通行（`docs/knowledge.md`
> 的「style 类别」）。
...
| replace_section 丢尾（v78 前） | `entry_render_report` 里 truncated 非空 + 同条目有 replace_section | 对比 apply 前后条目全文（v78 起无整节覆盖 op，此失效面已消失；历史 run 看知识库内嵌 git：`git -C knowledge log/show`） | pending 压缩 feature（docs/knowledge.md 遗留项） |
...
（注入截断风险）。修复归属：`finesub/llm/knowledge/node/proposals.py`（翻译）/ `node/apply.py`（引擎）或按提案格式手工应用（v78 起无内嵌 git）。

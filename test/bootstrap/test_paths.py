...
@pytest.mark.skipif(
    os.name != "nt",
    reason=r"drive-letter volumes: elsewhere `Z:\FineSub` is just a relative name",
)
def test_an_offline_volume_falls_back_without_touching_the_record(
    ...
    #
    # Windows-only by construction: `_anchor_exists` asks whether the *volume*
    # is present, and only a drive letter has one to be absent. On POSIX
    # `Z:\FineSub` parses as a relative path with an empty anchor, so the
    # fallback still happens -- via the "recorded directory is gone" branch,
    # with a different message -- and the assertion below would be checking
    # the wrong one.
    data_root = tmp_path / "data"
    data_root.mkdir()
...
def test_an_interrupted_relocate_still_finds_every_directory(tmp_path: Path) -> None:
    """Half-moved is a state the resolver has to survive, not a broken install.

    `relocate` records the destination before moving so nothing is ever
    unreachable; the cost is that a crash leaves some of models/cache/tasks at
    each root. Without the fallback the app would re-download the models and
    come up with an empty task list while the real one sat at the old root.
    """
    from finesub_bootstrap.paths import clear_migration_source, record_big_data

    root = tmp_path / "install"
    data_root = tmp_path / "data"
    destination = tmp_path / "elsewhere"

    old = AppPaths.for_root(root, data_root=data_root)
    ensure_store(old)
    (old.tasks / "job-1").mkdir(parents=True)
    (old.agent_capsules / "failed-call").mkdir(parents=True)

    # The move got as far as `models` and then the process died.
    new = old.with_big_data(destination)
    ensure_store(new)
    new.models.mkdir(parents=True, exist_ok=True)
    (new.models / "weights.bin").write_bytes(b"data")
    record_big_data(data_root, destination, migrating_from=old.big_data)

    resolved = load_app_paths(root, data_root=data_root)

    assert resolved.big_data == destination.resolve()
    assert (resolved.models / "weights.bin").is_file(), "models are at the new root"
    assert (resolved.tasks / "job-1").is_dir(), "tasks are still at the old one"
    assert (resolved.agent_capsules / "failed-call").is_dir()

    # Once the move is declared finished, resolving is root-level again.
    clear_migration_source(data_root)
    finished = load_app_paths(root, data_root=data_root)
    assert finished.tasks == (destination.resolve() / "tasks")


def test_a_finished_relocate_leaves_no_migration_marker(tmp_path: Path) -> None:
    from finesub_bootstrap.paths import (
        read_locations,
    ...
    data_root = tmp_path / "data"
    record_big_data(data_root, tmp_path / "new", migrating_from=tmp_path / "old")
    assert recorded_migration_source(data_root) == tmp_path / "old"

    clear_migration_source(data_root)

    assert recorded_migration_source(data_root) is None
    assert "migratingFrom" not in read_locations(data_root)

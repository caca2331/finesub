...
from scripts.make_cn_lock import (
    LockMismatch,
    assert_equivalent,
    main,
    rewrite,
...
def test_the_generator_writes_nothing_when_the_result_would_drift(
    ...
    monkeypatch.setattr("scripts.make_cn_lock.rewrite", drifting)
...
def test_an_index_url_becomes_the_base_its_files_sit_under() -> None:
    ...
    from scripts.make_cn_lock import _pypi_files_base
...
# The two shipped locks moved into `src/finesub_bootstrap` with the
# desktop split (2026-09-03): the CLI needs them and the desktop is
# leaving, so they no longer belong to a subdirectory that is going away.
PACKAGED = Path(__file__).resolve().parents[2] / "src" / "finesub_bootstrap"
CANONICAL_LOCK = PACKAGED / "pylock.win-py312.toml"
REGIONAL_LOCK = PACKAGED / "pylock.win-py312.cn.toml"

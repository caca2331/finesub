...
def t2s_converter() -> Any:
    ...
    if _opencc_t2s is None:
        try:
            ...
            _opencc_t2s = False
            print(
                "Warning: opencc is unavailable, so 繁→简 normalization is off "
                "and traditional/simplified spellings will no longer match; "
                'install the harness extra (pip install -e ".[harness]").',
                file=sys.stderr,
            )

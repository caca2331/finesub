...
Research Progress 台账（此前各轮增量的累积，可能为空）：
<research_progress>
$progress_log
</research_progress>

本地知识库索引（末轮为空；与未决 fact 相关的条目按 system 指令用 `<requested_entries>` 优先请求）：
<streamer_index>
$streamer_index
</streamer_index>
<common_index>
...
最后提醒（读完以上全部输入后）：先以 `<reasoning>` 块开头，随后输出 `<progress_update>`，然后二选一——继续检索（`<search_queries>`/`<extract_urls>` 至少一个非空，可附 `<requested_entries>`）或收尾（`<evidence_pack>`）；末轮必须收尾，非末轮仍有 priority ≥ 2 的未决 fact 时默认继续检索。不能因为输出较长而省略任何必需标签、fact 或固定章节；压缩措辞也必须严格按要求格式完整输出。块外不要输出任何其他文字。

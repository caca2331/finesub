...
The CLI shell builds on this package; nothing in it may depend on a UI.

Requires ``pydantic`` and ``httpx`` (``[dev]`` and ``[harness]`` provide them,
and the managed runtime's lock carries both) --
...
live in ``test/bootstrap``; the two files only Windows can really execute
(junctions, DPAPI) are run by name on the Windows job in ``ci.yml``.
"""

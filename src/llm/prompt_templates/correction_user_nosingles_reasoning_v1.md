...
4. **前置 reasoning 注释**：合并 ≥2 源 / discard / conf=low / 越硬门槛的行，**必须**在其正上方写一行 `# <局部推理>`（不写源序号，位置即归属）；纯单源在界内的行**不写**注释。注释分析该行与前后 1-2 行的 gap、语义、说话人，给出合并/拆分/丢弃依据。note 只写短结论，不重复推理。
5. `<asr_result>` 中每一行都是本次输出范围；只读 `<preceding_context>` 中的行一律不得输出。
...
最后提醒（读完以上全部输入后）：本窗口共有 **$current_asr_row_count 条输入字幕**。先以 `<reasoning>` 开头（只写全局判断：专名统一、语域、话题分段、高风险区间），再写 `<translated>` 完整终稿；translated 第一行必须是完整 9 列 header。$merge_reminder不能因为输出长而省略、缩写或用省略号替代任何必需 header 或字幕行：translated 必须给出完整终稿，覆盖本窗每个源（疑似幻觉以 `discard|<源序号>` 显式丢弃），并严格保持标签、header、九列字段与顺序；决策点（合并/丢弃/low/越界）必须有前置 `#` 注释，非决策点不写。"(此处省略…)""(内容已生成…)"一类占位说明等同于空输出，整窗作废。每行首列 type 必须写 `sub`，不要留空。其后 `<next_advice>`、`<keep_entries>`（均可空，及如需的 `<task_update_feedback>`）。不要输出 `plan|` 或其他文字。
